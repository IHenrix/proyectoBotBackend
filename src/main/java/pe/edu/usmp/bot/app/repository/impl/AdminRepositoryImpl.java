...
public class AdminRepositoryImpl extends JdbcDaoSupport implements AdminRepository {
    ...
    public PersonaResponse buscarPersona(Long idPersona) {
        try {
            String sql = "SELECT p.id, p.nombre, p.apellido_paterno, p.apellido_materno, p.sexo, p.codigo, p.email, p.telefono, p.carrera " +
                    "FROM persona p INNER JOIN usuario u ON p.id_usuario = u.id " +
                    "WHERE EXISTS (SELECT 1 FROM usuario_rol ur WHERE ur.usuario_id = u.id) AND p.id = ?";
            return jdbcTemplate.queryForObject(sql, new Object[]{idPersona}, BeanPropertyRowMapper.newInstance(PersonaResponse.class));
